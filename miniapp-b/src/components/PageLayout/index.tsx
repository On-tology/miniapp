...
const Main = (props: { children: ReactNode; className?: string }) => {
  return (
    <main
      className={twMerge(
        clsx('grow overflow-y-auto p-6 pt-3 flex justify-center items-center', props.className),
      )}
    >
      {props.children}
    </main>
...
const Footer = (props: { children: ReactNode; className?: string }) => {
  return (
    <footer className={twMerge('px-6 h-24 flex items-center justify-center pb-10 pt-5 border-t border-gray-200', clsx(props.className))}>
      {props.children}
